...
# as the interpolated value in 2D
...
# but gives you one frame delay, because it has to use the transforms before movment for the current physics frame, (i think?)
...
var last_transforms := [Transform2D(), Transform2D()]
var new_transform_index := 0
var initial_target_transform: Transform2D
...
func set_interpolated_transform():
	var newest_transform := last_transforms[new_transform_index] as Transform2D
	var older_transform := last_transforms[old_transform_index()] as Transform2D
